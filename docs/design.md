...
Sticky header: product title, source link, language toggle. Search is a full-width underline field in the main column, with rank tabs beneath it. Source filter (All / GitHub / X / TikTok) is a left rail on large screens and a left Sheet on small screens — not a floating chip. Footer: short directory note and launch-post pointer. No third-party logos or decorative marks.
